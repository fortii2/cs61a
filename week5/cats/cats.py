...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    matches = [str for str in paragraphs if select(str)]
...
def accuracy(typed, source):
    ...
    for i in range(len(typed_words)):
        if len(source_words) > i and typed_words[i] == source_words[i]:
            right += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    current_diff = diff_function(typed_word, current_word, limit)
    ...
    return current_word if current_diff <= limit else typed_word
    # END PROBLEM 5
...
def report_progress(typed, source, user_id, upload):
    ...
    need_report = {"id" : user_id, "progress" : result}
    printing = lambda d: print('ID:', d['id'], 'Progress:', d['progress'])
    printing(need_report)
