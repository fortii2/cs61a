class Account:
    ...
    def time_to_retire(self, amount):
        ...
        own, count = self.balance, 0
        while own < amount:
            own += own * self.interest
            count += 1
        return count
...
def duplicate_link(s, val):
    ...
    while current_link is not Link.empty:
        if current_link.first == val:
            new_node = Link(val, current_link.rest)
            current_link.rest = new_node
            current_link = current_link.rest.rest
        else:
